...
push: test
	$(call message, "Push image $(REGISTRY)/$(REPOSITORY):$(TAG)")
	set -e ; \
...
test: build
	$(call message, "Test image $(REGISTRY)/$(REPOSITORY):$(TAG)")
	set -e ; if [ -f 'test/run.bats' ]; then bats -t test/run.bats; break; fi

.build: . $(DEPS)
	$(call message, "Build image $(REGISTRY)/$(REPOSITORY):$(TAG)")
...
clean: stop
	$(call message, "Clean image $(REGISTRY)/$(REPOSITORY):$(TAG)")
	@$(RM) $(TAG)/.build
...
stop:
	$(call message, "Stop image $(REGISTRY)/$(REPOSITORY):$(TAG)")
